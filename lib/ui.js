 ...
 * @param {{ type: string, label: string, description: string, pattern: string }[]} levels
 *        Bump levels for the current version (see getScheme)
 * @returns {Promise<string>}
 */
export async function promptMenu(levels) {
    const width = Math.max(...levels.map((level) => level.label.length));
    const choices = [
        ...levels.map((level) => ({
